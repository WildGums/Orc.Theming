    ...
    private static Bitmap? ExtractVistaIcon(Icon? icon)
    {
        ...
        try
        {
            ...
            for (var iIndex = 0; iIndex < iCount; iIndex++)
            {
                ...
                var imageSize = BitConverter.ToInt32(srcBuf, SizeICONDIR + SizeICONDIRENTRY * iIndex + 8);
                var imageOffset = BitConverter.ToInt32(srcBuf, SizeICONDIR + SizeICONDIRENTRY * iIndex + 12);
